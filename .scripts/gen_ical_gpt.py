...
table = soup.find_all("table", {"class": "clCommonGrid"})[
    1
]  # The second table contains the match schedule
...
for row in rows:
    if "clBold" in row.get(
        "class", []
    ):  # Skip rows that are just group headers (Omgång X)
        continue
    ...
    # Parse date and time
    try:
        # Assume the time is CET
        event_datetime = cet.localize(datetime.strptime(date_str, "%Y-%m-%d %H:%M"))
    except ValueError:
